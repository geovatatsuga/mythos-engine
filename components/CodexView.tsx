...
import { BookOpen, ChevronDown, Clock, Copy, Download, Edit3, EyeOff, Globe2, LayoutGrid, MapPin, Plus, Radar, Scale, Search, Share2, Shield, Sparkles, Users } from 'lucide-react';
import { useLanguage } from '../LanguageContext';
import { buildWordHtmlDocument, slugifyFilename, triggerDownload } from '../utils/export';
...
const safeText = (value?: string | null) => repairMojibakeText(value);
...
export default function CodexView({ universe, onUpdateUniverse, initialSection = 'overview' }: { universe: Universe; onUpdateUniverse?: (universe: Universe) => void; isLoading: boolean; initialSection?: string }) {
  ...
  const buildCodexExportText = () => {
    ...
    const addEntryList = (title: string, entries: string[]) => {
      lines.push(title);
      if (entries.length === 0) {
        lines.push(lang === 'en' ? 'No entries yet.' : 'Nenhuma entrada ainda.');
      } else {
        lines.push(...entries);
      }
      lines.push('');
    ...
    lines.push(universe.name || (lang === 'en' ? 'Untitled Codex' : 'Codex sem título'));
    if (universe.subtitle?.trim()) lines.push(universe.subtitle.trim());
    lines.push('');

    addSection(lang === 'en' ? 'Overview' : 'Visão Geral', universe.description || universe.codex.overview || '');
    if (universe.codex.overview?.trim()) addSection(lang === 'en' ? 'AI Overview' : 'Overview para IA', universe.codex.overview);
    if (universe.notesPrivate?.trim()) addSection(lang === 'en' ? 'Private Notes' : 'Notas Privadas', universe.notesPrivate);

    addEntryList(
      t('codex.section.characters'),
      universe.characters.map((character, index) => [
        `${index + 1}. ${character.name}${character.role ? ` — ${character.role}` : ''}`,
        character.aliases?.length ? `${t('codex.label.aliases')}: ${character.aliases.join(', ')}` : '',
        character.faction ? `${lang === 'en' ? 'Faction' : 'Facção'}: ${character.faction}` : '',
        character.status ? `${lang === 'en' ? 'Status' : 'Status'}: ${character.status}` : '',
        character.bio ? `${lang === 'en' ? 'Bio' : 'Bio'}: ${character.bio}` : '',
        character.ghost ? `Ghost: ${character.ghost}` : '',
    ...
    addEntryList(
      t('codex.section.factionsRealm'),
      universe.codex.factions.map((entry, index) => [
        `${index + 1}. ${entry.title}`,
        entry.aliases?.length ? `${t('codex.label.aliases')}: ${entry.aliases.join(', ')}` : '',
        entry.content,
        '',
      ].filter(Boolean).join('\n'))
    );
    ...
    addEntryList(
      lang === 'en' ? 'Lore & Systems' : 'Lore & Sistemas',
      universe.codex.rules.map((entry, index) => [
        `${index + 1}. ${entry.title}${entry.ruleKind ? ` — ${ruleKindLabel(entry.ruleKind, lang)}` : ''}`,
        entry.aliases?.length ? `${t('codex.label.aliases')}: ${entry.aliases.join(', ')}` : '',
        entry.content,
        '',
      ].filter(Boolean).join('\n'))
    ...
    addEntryList(
      t('codex.section.timeline'),
      universe.codex.timeline.map((entry, index) => [
        `${index + 1}. ${entry.title}`,
        `${lang === 'en' ? 'Impact' : 'Impacto'}: ${timelineImpactLabel(entry.timelineImpact ?? 'medium', lang)}`,
        `${lang === 'en' ? 'Scope' : 'Escopo'}: ${timelineScopeLabel(entry.timelineScope ?? 'local', lang)}`,
        entry.content,
        '',
      ].filter(Boolean).join('\n'))
    );
  ...
  const handleExportCodex = (format: 'txt' | 'doc') => {
    try {
      const exportTitle = universe.name || (lang === 'en' ? 'Untitled Codex' : 'Codex sem título');
      const exportText = buildCodexExportText();
      const fileBase = slugifyFilename(exportTitle, 'mythos-codex');

      if (format === 'txt') {
        triggerDownload(exportText, `${fileBase}-codex.txt`, 'text/plain;charset=utf-8');
        setToast({ msg: lang === 'en' ? 'Codex exported as TXT.' : 'Codex exportado em TXT.', type: 'success' });
        return;
      }

      const docHtml = buildWordHtmlDocument(`${exportTitle} Codex`, exportText);
      triggerDownload(docHtml, `${fileBase}-codex.doc`, 'application/msword;charset=utf-8');
      setToast({ msg: lang === 'en' ? 'Codex exported as DOC.' : 'Codex exportado em DOC.', type: 'success' });
    } catch (error) {
      console.error('[Codex Export Error]', error);
      setToast({ msg: lang === 'en' ? 'Failed to export codex.' : 'Falha ao exportar o codex.', type: 'error' });
    }
  };

  const handleCopyCodex = async () => {
    try {
      await navigator.clipboard.writeText(buildCodexExportText());
      setToast({ msg: lang === 'en' ? 'Codex copied to clipboard.' : 'Codex copiado para a área de transferência.', type: 'success' });
    } catch (error) {
      console.error('[Codex Copy Error]', error);
      setToast({ msg: lang === 'en' ? 'Failed to copy codex.' : 'Falha ao copiar o codex.', type: 'error' });
    }
  };
    ...
    <div className="mx-auto max-w-[1500px]">
      <div className="mb-8 flex flex-col gap-4 border-b border-stone-200 pb-5 xl:flex-row xl:items-end xl:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Button variant="secondary" size="sm" onClick={() => handleExportCodex('txt')}><Download className="mr-2 h-3.5 w-3.5" />{lang === 'en' ? 'Export TXT' : 'Exportar TXT'}</Button>
          ...
          <div className="sticky top-6 space-y-3">
            ...
            <div className="rounded-2xl border border-stone-200 bg-paper p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="text-sm font-serif font-semibold text-stone-900">{lang === 'en' ? 'Export codex' : 'Exportar codex'}</h3>
                  <p className="mt-1 text-xs leading-relaxed text-stone-500">
                    {lang === 'en'
                      ? 'Download the full archive with overview, characters, factions, rules, and timeline.'
                      : 'Baixe o arquivo completo com visão geral, personagens, facções, regras e linha do tempo.'}
                  </p>
                </div>
                <span className="inline-flex items-center rounded-full border border-stone-200 bg-stone-50 px-2 py-1 text-[10px] font-bold uppercase tracking-[0.24em] text-stone-500">
                  {universe.characters.length + universe.codex.factions.length + universe.codex.rules.length + universe.codex.timeline.length}
                </span>
              </div>
              <div className="mt-4 grid gap-2">
                <Button onClick={() => handleExportCodex('txt')} className="w-full bg-stone-800 hover:bg-black text-white">
                  <Download className="mr-2 h-4 w-4 text-primary" />
                  {lang === 'en' ? 'Download TXT' : 'Baixar TXT'}
                </Button>
                <Button onClick={() => handleExportCodex('doc')} variant="secondary" className="w-full">
                  <BookOpen className="mr-2 h-4 w-4" />
                  {lang === 'en' ? 'Download DOC' : 'Baixar DOC'}
                </Button>
                <Button onClick={handleCopyCodex} variant="ghost" className="w-full">
                  <Copy className="mr-2 h-4 w-4" />
                  {lang === 'en' ? 'Copy archive' : 'Copiar arquivo'}
                </Button>
              </div>
            </div>
          </div>
