...
export default function Sidebar({ currentView, setCurrentView, universeExists, locked = false }: SidebarProps) {
  ...
  return (
    <nav className={`relative flex flex-col items-center gap-y-6 bg-surface p-4 border-r border-stone-200 shadow-sm z-20 ${locked ? 'cursor-not-allowed' : ''}`}>
      ...
      <div className="flex flex-col gap-y-4">
        {navItems.map(item => (
          <NavItem
            ...
            disabled={locked || (item.view !== 'dashboard' && !universeExists)}
          />
        ))}
      </div>
      ...
      {locked && (
        <div className="absolute inset-0 rounded-r-xl bg-white/18 backdrop-blur-[1px]" />
      )}
    </nav>
  );
}
