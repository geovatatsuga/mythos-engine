...
import { Plus, Edit, FileText, Sparkles, BookOpen, Microchip, UserCheck, Search, Save, Eye, RefreshCw, ArrowLeft, CheckCircle2, Globe, Copy, Download } from 'lucide-react';
...
export default function ChaptersView({ universe, onGenerateChapter: _legacyGenerate, onUpdateUniverse, isLoading: globalLoading }: ChaptersViewProps) {
    ...
    const downloadWholeNovel = (format: 'txt' | 'doc') => {
        if (universe.chapters.length === 0) {
            setToast({ msg: t('chapters.exportNovelEmpty'), type: 'error' });
            return;
        }
        ...
        try {
            const exportTitle = typeof universe.name === 'string' && universe.name.trim().length > 0
                ? universe.name.trim()
                : t('chapters.exportNovelUntitled');
            const exportText = buildNovelExportText();
            const fileBase = slugifyFilename(exportTitle, 'mythos-novel');

            if (format === 'txt') {
                triggerDownload(exportText, `${fileBase}.txt`, 'text/plain;charset=utf-8');
                setToast({ msg: t('chapters.exportNovelTxtDone'), type: 'success' });
                return;
            }

            const docHtml = buildWordHtmlDocument(exportTitle, exportText);
            triggerDownload(docHtml, `${fileBase}.doc`, 'application/msword;charset=utf-8');
            setToast({ msg: t('chapters.exportNovelDocDone'), type: 'success' });
        } catch (error) {
            ...
            <aside className="lg:col-span-1 h-full min-h-0 flex flex-col">
                ...
                <Card className="mt-4 p-4 bg-paper border-stone-200 shadow-sm">
                    ...
                    <div className="mt-4 grid gap-2">
                        <Button
                            onClick={() => downloadWholeNovel('txt')}
                            disabled={universe.chapters.length === 0}
                            className="w-full bg-stone-800 hover:bg-black text-white disabled:bg-stone-300 disabled:text-stone-500"
                        >
                            <Download className="mr-2 h-4 w-4 text-primary" />
                            {t('chapters.exportNovelTxtCta')}
                        </Button>
                        <Button
                            onClick={() => downloadWholeNovel('doc')}
                            disabled={universe.chapters.length === 0}
                            variant="secondary"
                            className="w-full"
                        >
                            <BookOpen className="mr-2 h-4 w-4" />
                            {t('chapters.exportNovelDocCta')}
                        </Button>
                        <Button
