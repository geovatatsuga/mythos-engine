...
export interface DirectorGuidance {
  ...
  rhetoricalBans?: string[];
  imageryCooldown?: string[];
  openingConstraint?: string;
  closingConstraint?: string;
...
export type LongformChapterFunction =
  | 'setup'
  | 'inciting_break'
  | 'lock_in'
  | 'complication'
...
export interface LongformMilestone {
  chapter: number;
  label: string;
  objective: string;
}

export interface LongformBlueprintChapter {
  chapterNumber: number;
  actIndex: number;
  function: LongformChapterFunction;
  goal: string;
  milestone?: string;
}
...
export interface LongformBlueprint {
  ...
  milestones: LongformMilestone[];
  chapterMap: LongformBlueprintChapter[];
}

export interface LongformProgressState {
  currentChapter: number;
  currentAct: number;
  currentFunction?: LongformChapterFunction;
...
export interface NarrativeMemory {
  ...
  stylePatternCooldown?: Record<string, number>;
...
export interface Universe {
  ...
  notesPrivate?: string;
  syncMeta?: SyncMeta;
  creationMode?: 'manual' | 'genesis' | 'autogen_longform';
  longformBlueprint?: LongformBlueprint;
  longformProgress?: LongformProgressState;
}
...
export interface ChapterGenerationParams {
  ...
  chapterFunction?: LongformChapterFunction;
  actIndex?: number;
